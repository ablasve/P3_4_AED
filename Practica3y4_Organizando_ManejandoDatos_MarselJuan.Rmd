...
pew_religion_tidy <- pew_religion %>% 
  gather("income","count",-religion, factor_key = T)
pew_religion_tidy %>% head
```
...
```{r}
raw_tidy <- raw %>% pivot_longer(paste("wk",1:76,sep=""),names_to= "week",values_to = "value", names_prefix = "wk")
head(raw_tidy)
```
...
```{r}
TB_tidy <- raw %>% 
  gather(key = "casos", value = "valores", -c(country,year)) %>% 
  separate(casos, into = c("sexo", "edad"), sep = 1)
head(TB_tidy)
```
...
```{r}
weather_tidy <- raw %>% 
  pivot_longer(paste("d",1:31,sep=""),names_to = "dia", names_prefix = "d", values_to = "values", values_drop_na = T) %>% 
  pivot_wider(names_from = element, values_from = values) %>% 
...
head(planes)
```
En el data frame `flights` se muestran los datos de los vuelos, identificando al avión que los realizó por el número de cola, mientras que en el data frame `planes`se muestran los datos del modelo de cada avión.
...
```{r }
denominaciones_sin_douglas <- grep("DOUGLAS",unique(planes$manufacturer), ignore.case = T, value = TRUE, invert = T)
planes.f <- planes %>% 
  filter(manufacturer %in% denominaciones_sin_douglas) %>% 
  filter(seats>=90,seats<=210) 
planes.f$manufacturer[grepl("AIRBUS", planes.f$manufacturer,ignore.case = T)] <- "AIRBUS COMP"

# comprobamos que el rango de asientos es el deseado
range(planes.f$seats) 
# comprobamos que no hay aviones de la compañía Douglas
grep("DOUGLAS",unique(planes.f$manufacturer), ignore.case = T, value = TRUE)

# comprobamos que la única denominación de AIRBUS es la deseada
unique(planes.f$manufacturer)
```
...
```{r }
flights.f <- inner_join(flights,planes.f, by = "tailnum")
...
```{r }
flights <- flights %>% select(-c(time_hour,grep("time$",names(flights),ignore.case = T,value = T)))
head(flights)
...
Distances_plane <- flights %>% 
  group_by(tailnum) %>% 
  summarise(Num_flights = n(), Mean_distance = mean(distance, na.rm = T), Total_distance = sum(distance, na.rm = T)) %>% 
...
load('data/billboard.Rdata')
raw_tidy <- raw %>% 
  pivot_longer(paste("wk",1:76,sep=""),names_to= "week",values_to = "value", names_prefix = "wk") 

vect <-  1:length(unique(raw_tidy$track))
names(vect) <- unique(raw_tidy$track) 

# creamos la columna id
raw_tidy <- raw_tidy %>% 
  mutate(id = vect[track])

# separamos
canciones <- raw_tidy %>% 
  select(id,artist,year,track,time) %>% 
  unique
canciones

ranking <- raw_tidy %>% 
  select(-c(artist,year,track,time))
ranking
```
...
```{r}
# inspeccionamos el resultado al que queremos llegar
str(table1)
# inspeccionamos el data frame de partida
str(table5)
intento_de_table1 <- table5 %>% 
...
```{r }
Distance_Manufacterer_model <- inner_join(Distances_plane,planes, by = "tailnum") %>% 
  arrange(desc(Total_distance)) %>% 
  select(Total_distance, manufacturer, model)
head(Distance_Manufacterer_model)
```
